...
class FasterWhisperSTT(stt.STT):
    ...
    def __init__(
        ...
        # faster-whisper/ctranslate2 will error on many CPU backends if asked for float16.
        # On macOS in particular, CPU float16 is typically not supported/efficient.
        if device in ("cpu", "auto") and compute_type == "float16":
            fallback: ComputeType = "int8" if platform.system() == "Darwin" else "float32"
            logger.warning(
                "Requested compute_type=float16 on %s; falling back to compute_type=%s",
                device,
                fallback,
            )
